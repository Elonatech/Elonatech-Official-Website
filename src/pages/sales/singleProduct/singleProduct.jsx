   ...
   useEffect(() => {
    const fetchData = async () => {
        try {
          ...
          document.title = `${res.data.product.name} - Elonatech Nigeria Limited`;
...
const fetchRecentlyViewedProducts = async () => {
  try {
    const res = await axios.get(`${BASEURL}/api/v1/product/products/recently-viewed`);
    const recentlyViewedProducts = res.data.recentlyViewedProducts.slice(0, 6);
    setRecentlyViewed(recentlyViewedProducts.filter(product => product._id !== id).slice(0, 5));
  } catch (error) {
...
const ProductCard = ({ product }) => (
  <div className="col-custom-5 mb-4">
    <div className="card h-100 d-flex flex-column">
      <LazyLoadImage
        alt={product.name}
        src={product.images && product.images.length > 0 ? product.images[0].url : 'placeholder-image-url.jpg'}
        effect="blur"
      ...
      <div className="card-body d-flex flex-column">
        ...
        <p className="card-text">₦ {Number(product.price).toLocaleString()}.00</p>
...
const ProductSection = ({ title, products }) => (
  <div className="container mt-5">
    ...
    const getMetaTags = () => {
      ...
      return {
          title: `${data.name} - Elonatech Nigeria Limited`,
...
<Helmet>
                ...
                <link rel="canonical" href={getMetaTags().url} />
                
                {/* Preload critical image */}
                <link rel="preload" href={getMetaTags().image} as="image" />
                ...
                <meta property="og:url" content={getMetaTags().url} />
...
<button onClick={handleGoBack} className="btn btn-outline-dark back-btn">
...
<div class="container py-5 mb-" >
 ...
 <div className="col-2" style={{backgroundColor:"white"}}>
  <div className="card border-0">
    ...
    >
      <ul className='list-unstyled text-start' style={{ paddingLeft: "0" }}>
        {image.map((item) => (
          <li key={item.id} className='pb-3' style={{ display: "flex", justifyContent: "flex-start" }}>
            <SwiperSlide className='pb- mt-2' style={{ paddingBottom: "60px", display: "flex", justifyContent: "flex-start" }}>
              <img
                src={item.url}
                className='img-fluid hto rounded border border-dark'
                style={{ height: "60px", width: "60px", objectFit: "cover", cursor: "pointer" }}
              />
            </SwiperSlide>
          </li>
        ))}
    ...
    {image.map((item) => (
        <div key={item.id}> 
         <SwiperSlide className='border-0'>
          ...
          <img   alt="" class="img-fluid" src={item.url}></img>
          </div>
        </SwiperSlide>
        </div>
    ))}
            ...
            <div className="container" style={{display: "none"}}>
                ...
                <div dangerouslySetInnerHTML={{__html: sanitizedDescription}}></div>
                    ...
                    <h4 className='fw-bold'>{data.name}</h4>
                     <hr />
                    <div class=" mt-0">
                       <p className=""><span className='text-dark fw-bold pe-2'>Brand:</span>{data.brand}</p>
                    </div>
                    <div class="d-flex">
                        <h4 className='mt-'>Total: ₦ {Number(data.price).toLocaleString()}.00</h4>
                        <p className='ms-5 mt-1'> <del className=''>₦ {data.odd}.00</del></p>
                    </div>
                    <p className='text-danger'>{data.quantity <= "0" ? 'Out of Stock' : 'In Stock'}</p>
                        ...
                        <div className="row justify-content-md-end mt-3">
                         ...
                         <div className="col-6">
                                  {currentAdmin ? (
                                    ...
                                    <Link to={`/product/${id}/update`}><i class="bi bi-pencil-square text-warning fs-3 me-4" style={{cursor:"pointer"}}></i></Link>
                                    <i class="bi bi-trash3 text-danger fs-3"  style={{cursor:"pointer"}}   onClick={handleDelete}></i>
                                    </div>
                                   </div>
                                  ) :(
                                    <div></div>
                                ...
                                <p className='text-dark mt-2 ms-4'>Call <a className='text-decoration-none text-red ' href="tel:234 901 454 452">+234 901 454 4520</a> To Order</p>
                                </div>
...
<div className="row ">
  ...
  <div class="description mt-2 mb-5 p-" dangerouslySetInnerHTML={{__html: DOMPurify.sanitize(data.description)}}></div>
    </div>
  </div>
  </div>
</div>
 : 
<div>
<div className="container mt-5" style={{backgroundColor:"#f1f1f2"}} >
<div className="row ">
  <div className="col-lg-12">
  <h4 className='fw-bold mb-3 mt-3'>Overview</h4>
  <hr className='text-danger' />
  <div className="card border-0" style={{backgroundColor:"#f1f1f2"}}>
...
{recentlyViewed.length > 0 && (
  <ProductSection
    title={<h4 className="fw-bold mb-3 mt-3">Recently Viewed Products</h4>}
